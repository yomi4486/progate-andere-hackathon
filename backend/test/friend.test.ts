...
describe("FriendRoute API", () => {
    ...
    it("should send a friend request successfully", async () => {
        ...
        expect(await res.json()).toEqual({ message: "Friend Request Successfully" });
    });
    ...
    it("should return 409 if request is already sent", async () => {
        ...
        expect(await res.json()).toEqual({ message: "Already Send Request" });
    });
    ...
    it("should reject a friend request successfully", async () => {
        ...
        expect(res.status).toBe(200);
        expect(await res.json()).toEqual({ message: "Request Rejected Successfully" });
    });
    ...
    it("should update friend request status successfully", async () => {
        ...
        expect(res.status).toBe(200);
        expect(await res.json()).toEqual({ message: "Request Successfully" });
    });
});
