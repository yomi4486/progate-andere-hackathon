...
export default function NewUserScreen() {
    ...
    const [displayName,setDisplayName] = useState<string>(`${user?.data.user.name}`);
    return (
        <View style ={{height:"100%"}}>
            <DefaultHeader title="プロフィールを作りましょう！" showSettingButton={false}/>
            <View style={profileStyles.profileContainer}>
                <View style={styles.IconContainer}>
                {user?.data.user.photo?<Image
            ...
            <Text style={StyleSheet.compose(profileStyles.friendsTitle,{paddingLeft:30,paddingTop:20})}>自己紹介</Text>
            <View style={{alignItems: 'center'}}>
                <View style={{flexDirection: 'row',alignItems: 'center',width: '80%',borderColor: '#cccccc',borderWidth: 1,borderRadius: 20,paddingHorizontal: 10,}}>
                    <TextInput
                        style={{textAlign: 'center',height: 40,padding:10,minHeight:60}}
                        placeholder="自己紹介"
                        multiline={true}
                        numberOfLines={4}
                        onChangeText={(input)=>{setIntriduction(input)}}
                    />
                </View>
            </View>
            <FloatingActionButton onPress={async()=>{
                try{
                    if(user?.data.user.photo && displayName.length != 0 && user?.data.idToken){
                        ...
                        throw Error("Icon photo or display name cannot be empty");
                    }
                }catch(e){
                    console.error(e)
                }                
