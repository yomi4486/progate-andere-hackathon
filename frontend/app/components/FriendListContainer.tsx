...
const FriendListContainer: React.FC<FriendListContainerProps> = ({ title, friends }) => {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={[styles.title, { color: activeFriends.length > 0 ? 'green' : 'gray' }]}>{title}</Text>
        <Text style={[styles.count, { color: activeFriends.length > 0 ? 'green' : 'gray' }]}>
          {friends.length}
        </Text>
      </View>
