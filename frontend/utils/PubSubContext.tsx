...
export const PubSubProvider: React.FC<{ children: React.ReactNode }> = ({
	...
	useEffect(() => {
	  ...
	  const statusInterval = setInterval(() => {
	    ...
	    client.publish(`${userId}/status`, JSON.stringify(status), { retain: true })
	  }, 3000)
	  ...
	  const cleanupInterval = setInterval(() => {
	    ...
	    setFriendStatusMap(prevMap => {
	      ...
	      Object.entries(newMap).forEach(([friendId, status]) => {
	        if (status.status === 'online' && now - status.lastUpdate > 5000) {
	          newMap[friendId] = { status: 'offline', lastUpdate: now }
	          hasChanges = true
	        }
	      })
